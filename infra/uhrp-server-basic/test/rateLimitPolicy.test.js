...
const {
  ...
  configureTrustProxy,
  rateLimitOptions
} = require('../out/src/security/rateLimitPolicy.js')
...
describe('rate-limit security policy', () => {
  ...
  test('enforces identity buckets and emits a stable 429 response', async () => {
    const app = express()
    app.use((req, _res, next) => {
      req.auth = { identityKey: req.header('x-test-identity') }
      next()
    })
  ...
  test('requires bounded numeric proxy and rate-limit configuration', () => {
    const app = express()
